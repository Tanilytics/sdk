...
describe('session/session-storage', () => {
  ...
  it('returns null for malformed or invalid session data', () => {
    ...
    store.set(SESSION_KEY, JSON.stringify({ sessionId: '', startedAt: 1, lastActivityAt: 1 }));
    expect(loadSession()).toBeNull();

    store.set(SESSION_KEY, JSON.stringify({ sessionId: 's1', startedAt: '1', lastActivityAt: 1 }));
    expect(loadSession()).toBeNull();
  });
